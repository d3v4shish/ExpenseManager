...
from PyQt6.QtWidgets import QApplication, QFrame, QMainWindow, QMessageBox, QScrollArea, QVBoxLayout, QWidget
...
class ExpenseManagerWindow(QMainWindow):
    ...
    def __init__(
        ...
        *,
        ...
        QTimer.singleShot(0, self._check_thunderbird_directory_on_launch)
    ...
    def _check_thunderbird_directory_on_launch(self) -> None:
        """Show a launch warning when Thunderbird parsing has no valid source."""

        if self._thunderbird_warning_shown:
            return
        result = self._thunderbird_validation_result()
        if not result or bool(result.get("valid")):
            return
        self._thunderbird_warning_shown = True
        self._show_thunderbird_warning(result)
    ...
    def _show_thunderbird_warning(self, result: dict[str, Any]) -> None:
        message = str(result.get("message", "")).strip() or "Expense Manager could not find a valid Thunderbird mailbox directory to parse."
        ...
        self._show_warning_dialog(
            "Thunderbird Directory Not Found",
            f"{message}\n\nOpen Expense Config and select a valid Thunderbird profile/account before parsing email.",
        )

    def _build_layout(self) -> None:
        """Create the scrollable card layout."""
...
def run_app_window(
    *,
    ...
    app.setDesktopFileName("expense-manager-pyqt")
